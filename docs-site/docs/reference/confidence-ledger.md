...
|---|---|---|
| Spare-stock policy | How many pre-flashed spare CAN boards should sit on the shelf for the board-swap replacement path, and under what `node_id` allocation discipline. Recorded as an open item of ADR-0008 (`docs/adr/0008-post-live-firmware-evolution-no-can-bootloader.md`): *"Spare-stock policy (ADR-0008 open item 4) [ops / commissioning] — how many pre-flashed spare boards sit on the shelf for the board-swap path, and at what `node_id` allocation discipline (ADR-0007 id space). Supports `docs/reflash-campaign-runbook.md` Path B."* No answer has been recorded anywhere yet. | CAN node board-swap replacement |
