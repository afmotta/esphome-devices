...
|------|---------|---------|--------|
| 2026-08-14 | 1.17 | Added the QuinLED-An-Quad as a second lighting LED-strip controller and **CAN actuator node** (ADR-0021), a 4-channel, WiFi-only sibling of the An-Penta-Plus that **reuses ADR-0020's `CAT_OUTPUT` slice unchanged**. Board scaffolding (`boards/an-quad.yaml` + `-wifi`; classic ESP32, four raw LEDC channels on GPIO16/17/5/19, no Ethernet PHY so no `-ethernet` variant or `enable_ethernet` toggle) and a lighting entry point (`devices/an-quad-1.yaml`) composing two `cwww` tunable-white strips over the four channels; WiFi + HA for normal control (`api: reboot_timeout: 0s`). The HA-down fallback joins the house bus via an external SN65HVD230 transceiver on the An-Quad's spare expansion-header GPIOs (not a QWIIC header — the An-Quad has none) and applies OUTPUT commands to its strips (`lighting/packages/an_quad_can.yaml`, structurally identical to `an_penta_can.yaml`), as a first-class registry node (node 103, new external-actuator profile `led-quad` beside `led-penta` in `generate_nodes.py` — reserves node_id + `node_map`/`map.json`/health, generates no node YAML). No protocol/binding/dispatch change — the `CAT_OUTPUT` slice, `BindingEntry`, and gateway fallback are reused verbatim; empty-manifest hash `d66767448ba37b2f` unchanged. The two CAN transceiver GPIOs are isolated as `aux_gpio_1`/`aux_gpio_2` substitutions (defaulted GPIO13/GPIO23) pending schematic confirmation at bring-up (quinled.info was unreachable from the build env). Registry artifacts regenerate cleanly; native/Python battery passes (esphome CLI not available in-env for the entry-point compile) | AI Assistant |
