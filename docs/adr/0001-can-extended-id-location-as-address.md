...
relatedDocuments:
  - docs/adr/0002-runtime-assignable-node-addressing-and-commissioning.md
  - docs/adr/0007-flat-node-id-with-central-meaning-map.md
  - docs/design-notes/canbus-implementation-rules.md
...
**Superseded by ADR-0007 (2026-06-06).** Accepted and implemented on 2026-06-04, then
superseded: once control was centralized (ADR-0003), location-in-the-ID lost its value (no
distributed actuators filter by location — the central controller maps any id), and the
onboarding requirement favoured a meaningless `node_id` + central meaning-map over
location-as-address. The Extended-ID implementation was merged (PR #6) and then **reverted
(PR #8)**; the project returns to a flat `node_id`. This record is retained for history. See ADR-0007.

**~~Accepted~~** — approved by Alberto on 2026-06-04. The open items in the
[Consequences](#consequences) section were resolved *as part of implementation* (they
