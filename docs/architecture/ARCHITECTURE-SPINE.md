...
sources:
  ...
  - docs/adr/0009-central-map-binding-manifest-system-of-record.md
  - docs/adr/0013-gateway-local-relays-single-click-fallback.md
  - docs/contracts/spec-map-json-contract/SPEC.md
---
...
- **Rule:** The repo contains exactly these named systems: `canbus/` (infrastructure), `lighting/` (application), `climate/` (application), `vesta/` (library) — plus the shared `registry/`, `boards/`, `libs/` (custom external components), the `devices/` composition layer, and `docs/` (the knowledge base: `adr/`, `architecture/`, `contracts/`, `design-notes/`, plus runbooks). Every new file lands inside exactly one of these homes; nothing system-owned lives ambient at root. A decision superseding an earlier one is a new ADR under `docs/adr/` that cites the superseded ADR id (ADRs are not rewritten in place).
