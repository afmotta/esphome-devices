...
- `components/` — reusable component packages which are included with `packages:` and expect `vars` (examples: `components/pid.yaml`, `components/mixing_valve.yaml`, `components/dual_pid.yaml`).
...
- IDs and names are generated by templates. Example: PID IDs use `pid_${circuit_slug}_${mode}`; sensors are named like `dallas_0x...`. Follow existing interpolation patterns.
