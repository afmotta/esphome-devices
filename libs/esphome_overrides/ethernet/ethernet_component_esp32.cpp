...
void EthernetComponent::ethernet_lazy_init_() {
  ...
  // ---------------------------------------------------------------------------
  // LOCAL FORK (esphome-devices): allow sharing this SPI host with `spi:`.
  // ---------------------------------------------------------------------------
  // On the LilyGO T-Connect Pro the W5500 and the onboard ST7796 panel are wired
  // to the SAME pads (SCLK 12 / MOSI 11 / MISO 13; W5500 CS 10, display CS 21).
  // An ESP32-S3 pad has exactly one output source, so putting them on two
  // different SPI hosts cannot work — whichever initialises last wins the pads
  // and the other device goes silent. They must share ONE controller and be
  // arbitrated by their distinct CS lines, which is what LilyGO's own Arduino
  // code does.
  //
  // ESPHome's `spi:` component (setup_priority BUS 1000) initialises the host
  // long before ethernet (WIFI 250) gets here, so this call now finds the bus
  // already up and returns ESP_ERR_INVALID_STATE. That is the success path when
  // sharing: the bus we want is already configured with the very same pins, and
  // esp_eth_mac_new_w5500() below only needs to spi_bus_add_device() onto it
  // (see w5500_custom_spi.cpp), which works fine on a bus owned by `spi:`.
  //
  // Any OTHER error is still fatal, and when nothing else claimed the host this
  // behaves exactly as upstream — so the fork is a no-op for unshared buses.
  err = spi_bus_initialize(host, &buscfg, SPI_DMA_CH_AUTO);
  if (err == ESP_ERR_INVALID_STATE) {
    ESP_LOGI(TAG, "SPI host %d already initialized (shared with `spi:`); adding W5500 as a device",
