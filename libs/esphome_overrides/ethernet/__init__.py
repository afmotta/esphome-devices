        ...
        for spi_conf in spi_configs:
            if (index := spi_conf.get(CONF_INTERFACE_INDEX)) is not None:
                ...
                if interface == spi_host:
                    # ----------------------------------------------------------
                    # LOCAL FORK (esphome-devices): sharing is the POINT here.
                    # ----------------------------------------------------------
                    # Upstream raises cv.Invalid at this line because two
                    # components both calling spi_bus_initialize() on one host
                    # fails. Our fork's ethernet_component_esp32.cpp treats the
                    # resulting ESP_ERR_INVALID_STATE as the success path and
                    # simply adds the W5500 as a device on the bus `spi:` set up.
                    #
                    # That is REQUIRED on the T-Connect Pro, where the W5500 and
                    # the onboard panel are wired to the same pads and therefore
                    # must live on one controller, arbitrated by CS 10 vs CS 21.
                    # Matching interfaces is now the correct configuration, so
                    # this is informational rather than fatal.
                    LOGGER.info(
                        "`ethernet` and `spi` share interface '%s' — W5500 will be added as a "
                        "device on the bus owned by `spi:` (esphome-devices local fork).",
                        interface,
                    )
